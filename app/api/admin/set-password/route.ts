...
import { getAppName, getCanonicalAppUrl } from '@/lib/appAuth'
...
export async function POST(req: NextRequest) {
  ...
  const targetUser = await findAuthUserByEmail(admin, normalizedEmail)
  ...
  await sendEmail(
    ...
    `<p>Hi,</p>
     <p>Your ${appName} account is ready. Here are your login details:</p>
     <table style="margin:16px 0;border-collapse:collapse;">
       <tr><td style="padding:4px 12px 4px 0;color:#666;">Email</td><td style="font-weight:600;">${normalizedEmail}</td></tr>
     ...
     <p><a href="${appUrl}/login" style="background:#3b82f6;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;">Log in to ${appName} →</a></p>
