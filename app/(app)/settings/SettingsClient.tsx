...
function ChangePasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setErrorMsg('')
    if (next.length < 8) { setErrorMsg('Password must be at least 8 characters.'); return }
    if (next !== confirm) { setErrorMsg('Passwords do not match.'); return }

    setState('saving')
    // Re-authenticate first
    const { data: { user } } = await supabase.auth.getUser()
    if (!user?.email) { setState('error'); setErrorMsg('Not logged in.'); return }

    const { error: signInErr } = await supabase.auth.signInWithPassword({ email: user.email, password: current })
    if (signInErr) { setState('error'); setErrorMsg('Current password is incorrect.'); return }

    const { error: updateErr } = await supabase.auth.updateUser({ password: next })
    if (updateErr) { setState('error'); setErrorMsg(updateErr.message); return }
