...
export const handler = async (event) => {
  ...
  if (companyErr || !company) {
    console.warn('[twilio-inbound] Unknown Twilio number:', To)
  ...
  // Uses the increment_unread() SQL function defined in 002_functions.sql.
  // If the RPC is not yet deployed, fall back to a plain update.
  if (conversation?.id) {
    const { error: rpcErr } = await supabase.rpc('increment_unread', {
      conversation_id: conversation.id,
    })
    if (rpcErr) {
      console.warn('[twilio-inbound] increment_unread RPC failed, falling back to update:', rpcErr.message)
      // Fallback: direct update (may have a brief race condition under high load)
      await supabase
        .from('conversations')
        .update({
  ...
  // 9. Notify staff of the new reply
  // TODO: send email to company.staff_notification_emails and company.albi_email
  // Subject: [job?.customer_name ?? From] — New SMS reply
  // Body: include From phone, job name (if matched), and the full message text.
  // Recommended: use a transactional email service (Resend, SendGrid, Postmark)
  // or a Supabase Edge Function with an SMTP integration.
